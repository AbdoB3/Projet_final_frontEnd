...
function Rating() {
// Function to generate a random user image URL
const getRandomUserImage = () => {
  const randomNumber = Math.floor(Math.random() * 100) + 1;
  return `https://randomuser.me/api/portraits/${randomNumber < 50 ? 'men' : 'women'}/${randomNumber}.jpg`;
}
  ...
  <section id="testimonials" aria-label="What our customers are saying" className="bg-slate-50 py-20 sm:py-32">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-2xl md:text-center">
        <h2  className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4 p-5">What Our Customers Are Saying</h2>
      </div>
      <ul role="list" className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:gap-8 lg:mt-20 lg:max-w-none lg:grid-cols-3">
        {/* Testimonial items */}
        {/* Replace this with your actual testimonial data */}
        {/* For now, generating three dummy testimonials */}
        {[1, 2, 3].map((index) => (
          <li key={index}>
              ...
              <li>
                <figure className="relative rounded-2xl bg-white p-6 shadow-xl shadow-slate-900/10">
                  ...
                  <blockquote className="relative">
                    <p className="text-lg tracking-tight text-slate-900">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce pulvinar magna non aliquam tincidunt. Etiam in ex vitae orci efficitur vestibulum ac sed nulla.</p>
                  </blockquote>
                  <figcaption className="relative mt-6 flex items-center justify-between border-t border-slate-100 pt-6">
                    ...
                    <div className="overflow-hidden rounded-full bg-slate-50">
                      <img alt="" className="h-14 w-14 object-cover" src={getRandomUserImage()} />
                    </div>
                  </figcaption>
                </figure>
              </li>
